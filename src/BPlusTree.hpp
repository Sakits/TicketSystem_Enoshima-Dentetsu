...
public:
    BPlusTree (const char* file_name, int cache_size = 0) : node_pool(file_name)
    {
        ...
        {
            node_pool.insert(size);
            Node initnode;
            node_pool.insert(initnode);
        }
        else
            node_pool.read(4, size);
    }

    ~BPlusTree() 
    {
        node_pool.write(4, size);
    }
    ...
    bool insert(const ull _key, int file_pos, bool unique = 0, int x = root_file_pos, Node* const faptr = nullptr)
    {
        Node now;
        node_pool.read(x, now);
        ...
        if (now.size > max_size)
        {
            ...
            if (~nxt.nxtptr) 
                node_pool.write(nxt.nxtptr + sizeof(int), nxt_pos);
            ...
            if (!faptr)
            {
                ...
                x = root_pos; node_pool.write(nxt_pos + sizeof(int), x);
                ...
                node_pool.write(root_pos, root);
                node_pool.insert(now);
                return 1;
            }
            else
            {
        ...
        node_pool.write(x, now);
        return 1;
    }

    bool erase(const ull _key, int file_pos = -1, int x = root_file_pos, Node* const faptr = nullptr)
    {
        Node now; node_pool.read(x, now);
        ...
        if (now.size < block_size)
        {
            ...
            if (~nxtptr)
            {
                ...
                if (nxt.size > block_size)
                {
                    ...
                    node_pool.write(now.nxtptr, nxt);
                }
                else
                {
                    ...
                    if (~nxt.nxtptr)
                    {
                        ...
                        node_pool.write(nxt.nxtptr + sizeof(int), x);
                    }
                    else now.nxtptr = -1;
            ...
            else if (~preptr)
            {
                ...
                if (pre.size > block_size)
                {
                    ...
                    node_pool.write(now.preptr, pre);
        ...
        if (~x) node_pool.write(x, now);

        return 1;
    }

    int query(const ull _key, int x = root_file_pos)
    {
        Node now; node_pool.read(x, now);
