...
private:
    ...
    int tot = 0, size = 0;
    int* last;
    struct edge{Value val; ull too; int pre;} *e;
public:

    // maxn 表示 insert 次数上限，_mod 表示哈希模数，最好是质数
    InnerUniqueUnorderMap(int _maxn, int _mod) : maxn(_maxn), mod(_mod) 
    {
        last = new int [mod];
        e = new edge [_maxn + 1];
    }

    ~InnerUniqueUnorderMap() {delete last; delete e;}

    inline void add(int x, ull y, const Value &val)
    {
        e[++tot] = (edge){val, y, last[x]}; last[x] = tot;
        size++;
    }
